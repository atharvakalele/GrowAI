# 💎 GoAmrita Healthcare Ads Intelligence System
## MASTER FEATURE REGISTRY v9.0 — "VIBRANIUM EDITION"
**Date:** 11 April 2026 | **Total Features:** 183 (182 active + W10 redirect)  
...
## Python & Libraries (UPDATED v8.0!)
```
Python:     3.11.x   (stable + best AI/ML compat. Config ready for 3.12+ future upgrade)
Flask:      3.1.3    (web panel)
pandas:     3.0.2    (data manipulation — UPDATED v8.0! PyArrow string default)
numpy:      2.4.4    (numerical)
scipy:      1.17.1   (A/B test significance — UPDATED v8.0! bugfix)
LightGBM:   4.6.0    (bid prediction, ACoS forecast, waste detection)
Prophet:    1.3.0    (time series forecasting — Meta, Jan 2026)
SQLAlchemy: 2.0.49   (ORM — DB-agnostic. PostgreSQL ready IF needed, 1 line change)
Alembic:    1.18.4   (schema migrations — UPDATED v8.0! plugin system, bulk reflection)

✅ ALL COMPATIBLE WITH EACH OTHER — Verified April 2026
  pandas 3.0.2 requires Python ≥3.11 ✅
  numpy 2.4.4 requires Python ≥3.11 ✅
  LightGBM 4.6.0 supports pandas 3.0 + numpy 2.x ✅
  Prophet 1.3.0 supports pandas 3.0 + numpy 2.x ✅
  Alembic 1.18.4 supports SQLAlchemy ≥1.4.23 ✅
  
🔮 FUTURE UPGRADE PATH (config change only):
  Python: 3.11 → 3.12/3.13 (when all AI libs fully tested)
  Database: SQLite forever unless 40+ sellers approved → then PostgreSQL (1 line change)
  All versions: pinned in requirements.txt
```

## Database
```
Primary & DEFAULT: SQLite (data/goamrita.db) — ALL PHASES!
  ├── Zero setup — Python built-in sqlite3
  ├── ~50,000 reads/sec, ~5,000 writes/sec
  ├── Single file, backup = file copy
  ├── Perfect for 1 seller, 300+ products
  ├── ⚠️ SQLite = DEFAULT even in Phase 4. No change unless approved.
  
ORM: SQLAlchemy 2.0.x (DB-agnostic)
  ├── Same Python code for any database
  └── Architecture READY for PostgreSQL (1 line change) — but NOT planned

🚨 PostgreSQL Migration — CONDITIONAL ONLY:
  ├── TRIGGER: 40+ sellers actively using the system
  ├── REQUIRES: Explicit Msir approval before migration
  ├── Tool: pgloader (SQLite → PostgreSQL)
  ├── ORM code stays IDENTICAL (just connection string change)
  └── Until 40+ sellers approved: SQLite FOREVER, no discussion needed

Schema Versioning: Alembic 1.13.x
  └── Track DB schema changes like git tracks code
```
...
## AI / LLM (UPDATED v8.0!)
```
Layer 3 — 3 FREE Gemini Options (User selectable in Settings menu):

  Option 1: Gemini 2.5 Flash (DEFAULT ✅ Recommended)
    ├── Status: Stable GA
    ├── Free tier: 250 RPD, 10 RPM
    ├── Best for: Complex insights, ad analysis, recommendations
    └── Use when: Quality matters most

  Option 2: Gemini 2.5 Flash-Lite
    ├── Status: Stable GA
    ├── Free tier: 1,000 RPD, 15 RPM ← 4x more requests!
    ├── Best for: Simple classification, tagging, bulk processing
    └── Use when: Volume matters most (lighter model, faster)

  Option 3: Gemini 2.5 Pro
    ├── Status: Stable GA
    ├── Free tier: 100 RPD, 5 RPM
    ├── Best for: Deep analysis, complex reasoning
    └── Use when: Accuracy matters most (heaviest, slowest)

  ⚠️ Gemini 2.0 Flash DEPRECATED June 1, 2026 — DO NOT USE!
  ⚠️ Gemini 3.x Flash = PREVIEW only (not stable) — DO NOT USE in production!
  🔮 When Gemini 3.x becomes stable GA → migrate (model name change in config)

Layer 3 Fallback: Ollama (local LLM, ₹0, needs 8GB+ RAM)

Architecture: Model name = config variable → 1 line change to upgrade
  gemini_model: "gemini-2.5-flash"  # change here when upgrading
```

## 360° API Coverage Summary
```
Total Features: 183 (182 active)
...
│   ├── layer1_rules.py              ← Rule-based AI (Day 1)
│   ├── layer2_ml.py                 ← LightGBM 4.6.0 + Prophet 1.3.0 (Phase 2)
│   ├── layer3_llm.py                ← Gemini 2.5 Flash / Ollama (Phase 3, optional) ← UPDATED v4.0!
...
## Layer 2: Statistical/ML AI (Phase 2)
```
Technology: LightGBM 4.6.0 + Prophet 1.3.0 + pandas 2.2.x + numpy 2.4.4 + scipy 1.17.0
Cost: ₹0 (runs locally)
...
scipy 1.17.1: A/B test statistical significance, confidence intervals ← UPDATED v8.0!
```

## Layer 3: LLM AI (Phase 3, OPTIONAL) — UPDATED v8.0!
```
3 FREE Options (user selectable in Settings):
  DEFAULT: Gemini 2.5 Flash (250 RPD, 10 RPM) — best balance
  VOLUME:  Gemini 2.5 Flash-Lite (1,000 RPD, 15 RPM) — bulk tasks
  QUALITY: Gemini 2.5 Pro (100 RPD, 5 RPM) — deep analysis

  ⚠️ Gemini 2.0 Flash DEPRECATED June 1, 2026 — DO NOT USE!
  ⚠️ Gemini 3.x = PREVIEW only — DO NOT USE in production!
  🔮 Gemini 3.x stable → migrate (1 line config change)
  
Fallback: Ollama (local LLM, ₹0, needs 8GB+ RAM)
Cost: Free to ~₹2-3/day if exceeds free tier

Used for: Human-language insights, complex pattern explanation
  "This keyword is losing because competitor launched same product
   3 days ago, CPC increased 40%. Recommend: reduce bid 15%"
...
# 📋 COMPLETE FEATURE REGISTRY (153+ Features)
...
| C12 | Negative Keyword Engine | Auto-detect waste terms from search reports | P1 | 1 |
...
| C15 | Bid Optimizer Engine | Rule-based + profit-based bid adjustments | P1 | 1 |
...
## CATEGORY 2: 🧠 AI INTELLIGENCE LAYER (32 Features)
...
| I05 | Post-Change Response Monitor | Observe → Analyze → Continue/Reverse/New Strategy | P1 | 2 |
...
| I23 | Permanent Rule Maker | Proven rules auto-saved, applied next time directly | P1 | 2 |
| I24 | Cross-Product Learner | Apply learnings from product A to similar product B | P2 | 2 |
| I25 | 4-Level AI Learning Engine | Software→Category→Product→Keyword learning hierarchy. 3 core + extensible (Keyword/Account/Supplier). Marketplace tag mandatory. IQ Score per level. | P1 | 1 | ← NEW v6.0! |
...
| P04 | Stock-Out Ads Pause | Low stock → Option A: reduce to Profitable mode | P1 | 2 |
...
| P07 | Buy Box Monitor & Auto-Price Recovery | Lost Buy Box → configurable bid reduction + auto price decrease (default 5%/30min) + recheck + alert + min floor | P1 | 2 |
...
| P10 | Negative Review Ad Brake | 100% auto: negative ratio + 1★ spike + themes + Gemini refinement (NO manual, NO ACR API, NO keyword tracking) | P1 | 2 |
...
## CATEGORY 4: 🎯 COMPETITOR & MARKET INTELLIGENCE (11 Features)
...
| M04 | Market Keyword Gap Finder | 4-approach composite (Search Term Reverse + Category Discovery + Amazon Suggestions + Brand Analytics) — NO automation, recommend only, user approval required for every keyword action | P3 | 3 |
...
| M09 | BSR Trend Tracker | Primary: Manual sheet (ASIN|BSR|DATE), Fallback: SP-API Catalog salesRanks, Support: sales/traffic/CTR data. Keepa REMOVED. | P3 | 3 |
...
## CATEGORY 5: 📈 ORGANIC & GROWTH ENGINE (15 Features)
...
| G08 | New Product Launch Autopilot | Automated 30-day launch sequence (configurable). **Auto Deal/Coupon creation on launch for visibility boost** | P2 | 2 | ← ENHANCED v9.0! |
...
## CATEGORY 7: 📊 REPORTING & UI (16 Features)

| # | Feature | Description | Priority | Phase |
|---|---------|-------------|----------|-------|
| R01 | Daily Excel Report (Multi-Tab) | Report tabs (read) + Action tabs (tool reads back) | P1 | 1 |
...
| # | Feature | Description | Priority | Phase |
|---|---------|-------------|----------|-------|
| A01 | Priority Alert Engine | P1=urgent/money loss, P2=important, P3=insight | P1 | 1 |
...
| A13 | 4-Type Alert Engine | Event-based + % Change + Fixed Value Change + Time-based alerts per module. **Preset: Sales drop >50% in 24hrs → P1 alert (Sales Velocity Alert)** | P1 | 2 | ← ENHANCED v9.0! |
...
## CATEGORY 9: ⚙️ AUTOMATION & WORKFLOW (12 Features)
...
## CATEGORY 10: 🔌 INTEGRATION & EXTENSIBILITY (17 Features)

| # | Feature | Description | Priority | Phase |
|---|---------|-------------|----------|-------|
| E01 | JSON Config System (3-Level) | Account → Category → Product inheritance, no hardcoding | P1 | 1 |
| E02 | Web Config Panel (Flask 3.1.3) | localhost + server same code, 10 pages, 30+ tabs | P2 | 2 |
...
| E06 | Multi-Account Support | **One login for user** — self + partner accounts. First-fill = master, auto-sync to sub-accounts, each editable. One seller can have multiple accounts (same/different marketplace). | P3 | 4 | ← ENHANCED v6.0! |
...
## CATEGORY 11: 🔑 KEYWORD SOURCE MODES & TOOLS (9 Features — v9.0 UPDATED!)
...
*MASTER Feature Registry v9.0 | 11 April 2026*
*183 Features / 182 active (15 new in v9.0, count corrected from earlier versions) | 11 Categories | 4 Build Phases | API Versions LOCKED*
*Changes from v8.0: Round 9 — 15 NEW features (P17-P18, W11-W12, K09, G13-G15, M11, I29-I32, H11, R16) + 11 existing feature improvements (C17, C18, M01, M03, G08, G09, K07, H08, H09, W05, H01) + A13 preset + W10→G09 cleanup*
